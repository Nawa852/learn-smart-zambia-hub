...
export const HubPageLayout: React.FC<HubPageLayoutProps> = ({
  ...
  return (
    <div className="space-y-0">
      {/* Hero */}
      <div className="relative overflow-hidden rounded-2xl border border-border/40 bg-card mb-4 lg:mb-6">
        ...
        <div className="relative px-4 py-4 lg:px-6 lg:py-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            <div className="shrink-0 flex items-center justify-center w-10 h-10 lg:w-11 lg:h-11 rounded-xl bg-primary/10 text-primary">
              <Icon className="w-5 h-5 lg:w-5.5 lg:h-5.5" />
            </div>
            <div>
              <h1 className="text-lg lg:text-xl font-bold text-foreground">{title}</h1>
              <p className="text-[11px] lg:text-xs text-muted-foreground mt-0.5 max-w-lg">{subtitle}</p>
            </div>
          </div>
            ...
            <div className="hidden sm:flex flex-wrap gap-2">
              {quickLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-secondary/80 hover:bg-secondary text-xs font-medium text-secondary-foreground transition-colors border border-border/30"
                >
      ...
      <Tabs value={tab} onValueChange={setTab} className="w-full">
        <div className="sticky top-12 lg:top-14 z-30 -mx-4 px-4 lg:-mx-6 lg:px-6 py-2 bg-background/90 backdrop-blur-xl border-b border-border/30">
          <TabsList className="w-full justify-start gap-1 bg-transparent h-auto p-0 overflow-x-auto scrollbar-none">
            {tabs.map((t) => (
              <TabsTrigger
                ...
                value={t.id}
                className={cn(
                  "relative gap-1.5 px-3.5 lg:px-4 py-2 rounded-full text-xs lg:text-sm font-medium transition-all whitespace-nowrap shrink-0",
