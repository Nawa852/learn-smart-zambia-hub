...
export const GamificationSetup = ({ onComplete }: GamificationSetupProps) => {
  ...
  const themes = [
    { id: 'explorer', icon: Sparkles, label: "Knowledge Explorer", description: "Discover new territories of learning", color: "from-blue-500 to-cyan-500" },
    { id: 'mastermind', icon: Crown, label: "Mastermind", description: "Conquer every challenge strategically", color: "from-purple-500 to-pink-500" },
    { id: 'scholar', icon: Trophy, label: "Scholar Sage", description: "Pursue wisdom and excellence", color: "from-green-500 to-emerald-500" },
    { id: 'whisperer', icon: Zap, label: "AI Whisperer", description: "Master the art of AI collaboration", color: "from-orange-500 to-red-500" },
  ];
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      <div className="text-center space-y-2">
        ...
        <h2 className="text-3xl font-bold text-primary">Gamified Soul Setup</h2>
        ...
        <RadioGroup value={selectedTheme} onValueChange={setSelectedTheme} className="grid md:grid-cols-2 gap-4">
          {themes.map((theme) => (
            <motion.div key={theme.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <Card className={`cursor-pointer border-2 transition-all ${selectedTheme === theme.id ? 'border-primary shadow-card-hover' : 'border-border hover:border-primary/50'}`}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-4 md:grid-cols-8 gap-4">
          {avatarOptions.map((avatar) => (
            <motion.div key={avatar.id} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setSelectedAvatar(avatar.id)}
              className={`cursor-pointer ${selectedAvatar === avatar.id ? 'ring-4 ring-primary rounded-full' : ''}`}>
        ...
        <p className="text-sm text-muted-foreground">Your avatar will grow and evolve as you progress</p>
      </div>

      <Card className="bg-primary/5 border-primary/20">
        <CardContent className="p-6">
          <h3 className="font-semibold mb-4 flex items-center gap-2"><Trophy className="w-5 h-5 text-primary" /> Your Starting Rewards</h3>
          <div className="space-y-3">
            {rewards.map((reward, index) => (
              <motion.div key={reward.label} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }}
          ...
          <div className="mt-4 p-4 bg-primary/5 rounded-lg border border-primary/20">
            <p className="text-sm text-center"><span className="font-semibold">Every action earns resonance</span> — XP, EduCoins, and respect</p>
          </div>
        </CardContent>
      </Card>

      <div className="flex justify-center">
        <Button onClick={() => onComplete({ theme: selectedTheme, avatar: selectedAvatar })} size="lg" className="min-w-48">
